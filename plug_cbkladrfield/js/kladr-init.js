(function ($) {
	$(function () {
		var panel = $('#' + window.kladr_field_id + '-panel');

		var token = '51dfe5d42fb2b43e3300006e';
		var key = '86a2c2a06f1b2451a87d05512cc2c3edfdf41969';

		var city = panel.find('[name="city"]');
		var street = panel.find('[name="street"]');
		var building = panel.find('[name="building"]');
		var buildingAdd = panel.find('[name="building-add"]');

		var addressFullContainer = $('#' + window.kladr_field_id);
...
//            $('#address').text(address);
			addressFullContainer.val(address);
		}

		// Обновляет лог текущего выбранного объекта
		var Log = function (obj) {
			var logId = panel.find('.addition .id');
			if (obj && obj.id) {
			...
			var logName = panel.find('.addition .name');
			if (obj && obj.name) {
			...
			var logZip = panel.find('.addition .zip');
			if (obj && obj.zip) {
			...
			var logType = panel.find('.addition .type');
			if (obj && obj.type) {
			...
			var logTypeShort = panel.find('.addition .type_short');
